"""A collection of all the methods used to save the objects and the settings of the game"""
import os
import pickle
...
def save_game(player, enemies, coins_removed, score, button):
    ...
    with open(GAME_SAVE_PATH, "wb+") as game_save:
        pickle.dump({'player_coords': player, 'enemies_coords': enemies,
                     'coins_removed': coins_removed, 'score': score}, game_save)
...
def continue_game():
    ...
    return old_save['player_coords'], old_save['enemies_coords'],  old_save['coins_removed'], old_save['score']
